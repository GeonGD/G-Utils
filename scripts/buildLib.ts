...
ii(async function main() {
  ...
  _.ls('-d', 'lib/*').forEach(file => {
    if ((file.endsWith('.d.ts') || file.endsWith('.json') || fs.statSync(file).isDirectory()) && (!file.endsWith('index.d.ts'))) {
      _.rm('-rf', file)
    }
  })
  ...
  _.ls('lib/*.js').forEach(file => {
    const reg = new RegExp(escapeRegExp(biliConfig.babel && biliConfig.babel.objectAssign || 'Object.assign'), 'g')
    _.sed('-i', reg, assign.name, file)
  })
})
