import { AnyObject } from '../Common/enhanceType'
 ...
 * 但在运行时 `key` 始终未 `string`
 * 因此, 你应该始终把 `key` 当成 `string` 处理
...
export function forOwn<T extends AnyObject>(obj: T, traverse: ForOwnTraverse<T>) {
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      if (traverse(obj[key] as T[Extract<keyof T, string | number>], key as any, obj) === false) {
        break
      }
    }
  }
